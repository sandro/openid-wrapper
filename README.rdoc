...
Handle openid completion the same way that the Rails open_id_authentication does it.
...
Add to config/routes.rb:
  ...
  map.resource :session, :collection => {:complete => :get}
  ...
  def complete
    complete_openid do |result, identity_url, registration|
      if result.successful?
        ...
        failed_login(result.message || "Sorry, open id authentication failed.")
      end
    end
  end
...
1. Priit Tamboom
2. http://svn.rubyonrails.org/rails/plugins/open_id_authentication/
3. http://code.google.com/p/open-id-authentication/
4. http://gitorious.org/projects/openid-wrapper/ 
